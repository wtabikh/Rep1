...
%onsets = {};
open(ONSETS, "<$scriptdir/$language-ValidOnsets.txt") or die("Couldn't open $scriptdir/input/$language-ValidOnsets.txt\n");
...
while(defined($fileline = <CORPUS>)){
    ...
    while(@wordarray > 0){
        ...
        while(@chararray > 0){
			...
			# if hit a vowel..
            if($currchar =~ /[ɨaeàèãioũẽuùaĩ]/){
                if(@chararray[@chararray-1] !=~ /[ɨaeàèãioũẽuùaĩ]/){
